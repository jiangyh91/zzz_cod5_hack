...
init()
{
	...
	set_zombie_var( "zombie_powerup_berserk_on", 	false );
	...
	set_zombie_var( "zombie_powerup_berserk_time", 	30 );
	set_zombie_var( "zombie_powerup_drop_increment", 	1 );	// lower this to make drop happen more often
	set_zombie_var( "zombie_powerup_drop_max_per_round", 500 );	// lower this to make drop happen more often
...
//	add_zombie_powerup( "nuke", 		"zombie_bomb",		&"ZOMBIE_POWERUP_NUKE", 			"misc/fx_zombie_mini_nuke_hotness" );
	...
	// New
	add_zombie_powerup( "berserk",  	"zombie_brain",	"BERSERK");
	//add_zombie_powerup( "firedamage",  	"zombie_perk_bottle_doubletap",	&"ZOMBIE_POWERUP_MAX_AMMO");

	// perks
	add_zombie_powerup( "perk", "zombie_perk_bottle_doubletap", "Perk" );
...
powerup_drop(drop_point,zombie)
{
				...
				else
				{
					switch (self.powerup_name)
					{
						...
						case "insta_kill":
							...
							break;	
						case "perk":
							players[i] thread maps\_rampage_util2::find_perk();
							break;
...
//	array_thread(level,::point_doubler_on_hud, drop_item);
	...
	if(randomint(100) > 75)
	{
		level.xpScale = level.xpScale*2;
	}
	
	level.zombie_vars["zombie_point_scalar"] = 2;
	wait 30;
	
	level.xpScale = level.saved_xpScale;
...
time_remaining_on_berserk_powerup()
{
	self endon("disconnect");
	self endon("death");
	
	self.berserk_hud = maps\_hud_util::createFontString( "objective", 2, self );
	self.berserk_hud maps\_hud_util::setPoint( "TOP", undefined, 0, level.zombie_vars["zombie_timer_offset"] + level.zombie_vars["zombie_timer_offset_interval"] + 30);
	self.berserk_hud.sort = 0.5;
	self.berserk_hud.alpha = 0;
	self.berserk_hud fadeovertime(0.5);
	self.berserk_hud.label = "Berserk: ";
	self.berserk_hud.alpha = 1;
	
	berserk_time = 30;
	self.berserk_hud setvalue( berserk_time );
	
	temp_ent = spawn("script_origin", (0,0,0));
	temp_ent playloopsound ("double_point_loop");

	// time it down!
	while ( berserk_time > 0)
	{
		wait 1;
	...
	self playsound("points_loop_off");
	temp_ent stoploopsound(2);
	
	self.berserk_hud destroy();
	temp_ent delete();
}
...
berserk_powerup()
{
	...
	self notify( "berserk instakill" );
	self endon( "berserk instakill" );
	...
	self visionSetNaked("berserker",0.1);
	self setClientDvar("g_speed",(getDvarInt("g_speed")+35));
	level.berserk = true;
	self.berserk = true;
	self thread time_remaining_on_berserk_powerup();
	wait( 30 );
	self.berserk = false;
	level.berserk = false;
	self setClientDvar("g_speed",190);
	self visionSetNaked("zombie",0.1);
	self notify("berserk end");
}
...
check_for_instakill( player )
{
	...
	{
		self maps\_zombiemode_spawner_prototype::zombie_head_gib();
