...
#include maps\_music; 
#include maps\factory\_zombiemode_utility; 
...
////---------------------------------------////
	...
	maps\factory\_zombiemode_weapons::init();
	maps\factory\_zombiemode_blockers_new::init();
	maps\factory\_zombiemode_spawner::init();
	maps\factory\_zombiemode_powerups::init();
	maps\_zombiemode_radio::init();	
	maps\factory\_zombiemode_perks::init();
	maps\factory\_zombiemode_tesla::init();
	maps\factory\_zombiemode_dogs::init();
...
//	level thread coop_player_spawn_placement();
	...
	//	level.global_kill_func = maps\factory\_zombiemode_spawner::zombie_death; 
	level.global_damage_func = maps\factory\_zombiemode_spawner::zombie_damage; 
	level.global_damage_func_ads = maps\factory\_zombiemode_spawner::zombie_damage_ads;
	level.overridePlayerKilled = ::player_killed_override;
	level.overridePlayerDamage = ::player_damage_override;
	
	level.melee_miss_func = maps\factory\_zombiemode::zombiemode_melee_miss;
...
track_players_ammo_count()
{
	...
	while(1)
	{
		...
		for(i=0;i<players.size;i++)
		{
			...
			if(!isDefined(weap) || weap == "ammo_bag" || weap == "none" || weap == "zombie_perk_bottle_doubletap" || weap == "zombie_perk_bottle_jugg" || weap == "zombie_perk_bottle_revive" || weap == "zombie_perk_bottle_sleight" || weap == "mine_bouncing_betty" || weap == "syrette" || weap == "zombie_knuckle_crack" || weap == "zombie_bowie_flourish" )
			{
				continue;
			}
...
add_low_ammo_dialog()
{
	index = maps\factory\_zombiemode_weapons::get_player_index(self);	
	player_index = "plr_" + index + "_";
	if(!IsDefined (self.vox_ammo_low))
	{
		num_variants = maps\factory\_zombiemode_spawner::get_number_variants(player_index + "vox_ammo_low");
		self.vox_ammo_low = [];
	...
	self maps\factory\_zombiemode_spawner::do_player_playdialog(player_index, sound_to_play, 0.25);	
...
add_no_ammo_dialog( weap )
{
	...
	{
		num_variants = maps\factory\_zombiemode_spawner::get_number_variants(player_index + "vox_ammo_out");
		self.vox_ammo_out = [];
		for(i=0;i<num_variants;i++)
		{
...
spawn_vo()
{
	...
	if(players.size > 1)
	{
		...
		index = maps\factory\_zombiemode_weapons::get_player_index(player);
...
init_levelvars()
{
	...
	zbrt = 10;
	zspd = 2;
	zhi = randomIntRange(100,200);
	zhip1 = randomIntRange(10,20);
	zhip2 = randomIntRange(110,170);
	zhs = randomIntRange(150,170);
	zss = 0;
	zsk = 0;
	zsd = 0;
	zsbm = 0;
	...
	else if( getDvarInt("g_gameskill") == 1 )
	{
		zrt = 5;
		zbrt = 8;
		zspd = 1.5;
	...
	else if( getDvarInt("g_gameskill") == 2 )
	{
		zrt = 35;
		zbrt = 7;
		zspd = 1;
		zhi = randomIntRange(200,240);
		zhip1 = 20;
		zhip2 = 100;
		zhs = randomIntRange(190,210);
		zss = 100;
		zsk = 60;
		zsd = 2;
		zsbm = 7;
	...
	else if( getDvarInt("g_gameskill") == 3 )
	{
		zrt = 40;
		zbrt = 5;
		zspd = 0.7;
		zhi = randomIntRange(230,280);
		zhip1 = 25;
		zhip2 = 110;
		zhs = randomIntRange(200,230);
		zss = 0;
		zsk = 50;
		zsd = 1;
		zsbm = 5;
	...
	// Scoring
	level.zombie_vars["zombie_round_time"] = 				zrt;
	level.zombie_vars["zombie_between_round_time"] = 		zbrt;
	level.zombie_vars["zombie_spawn_delay"] = 				zspd;
	level.zombie_vars["zombie_health_increase"] = 			zhi;
	level.zombie_vars["zombie_health_increase_percent"] = 	zhip1 / zhip2;
	level.zombie_vars["zombie_health_start"] = 				zhs;
	level.zombie_vars["zombie_score_start"] = 				zss;
	level.zombie_vars["zombie_score_kill"] = 				zsk;
	level.zombie_vars["zombie_score_damage"] = 				zsd;
	level.zombie_vars["zombie_score_bonus_melee"] = 		zsbm;
	level.zombie_vars["zombie_score_bonus_head"] = 			zsbh;
	level.zombie_vars["zombie_score_bonus_neck"] = 			zsbn;
	level.zombie_vars["zombie_score_bonus_torso"] = 		zsbt;
	level.zombie_vars["zombie_score_bonus_burn"] = 			zsbb;

	level.zombie_vars["penalty_no_revive_percent"] = 		10 / 100;
	level.zombie_vars["penalty_died_percent"] = 			0 / 100;
	level.zombie_vars["penalty_downed_percent"] = 			5 / 100;	
...
player_is_cheating()
{
	...
	if( getDvarInt( "g_speed" ) > 190 && !level.berserk && !level.f_run)
	{
		level.cheating = true;
		level.cheating2 += 1;
	}
	...
	if( getDvarInt( "arcademode" ) >= 1)
	{
		level.cheating = true;
		level.cheating2 += 1;
	}
	
	return level.cheating;
	return level.cheating2;
}
...
cheat_hud()
{
	...
	while(1)
	{
		cheats_on = player_is_cheating();
		if( cheats_on )
		{
			level.ranks_on = false;
			hud SetText( "^1CHEATS BEING USED" ); 
			hud.alpha = 1;
		}
		else
		{
...
onPlayerDisconnect()
{
	...
	maps\menu_classes::set_clientdvars_2(self.class_picked);
}
...
onPlayerSpawned()
{
	...
	for( ;; )
	{
		...
		self thread maps\_rampage_util::rank_progress_bar();
		...
		if(!isdefined(self.hud_rankscroreupdate))
		{
			...
			self.hud_rankscroreupdate maps\_rampage_util::fontPulseInit();
		}
	}
}
...
#/
	...
	if(!level.tom_use_timer)
	{
		...
		if(getdvarint("realism_bosses") == 1)
		{
			...
			{
				maps\_rampage_util::calculate_boss_stuff();
				level.bosses_enabled = true;
				level.boss_round = level.round_number + 4;
				iprintlnbold("BOSSES: "+level.bosses);
			}
		}
	}
	...
	if(!level.tom_use_timer)
	{
		while( count < max )
		{
			...
			// MM Mix in dog spawns...
				...
				{
					spawn_dog = false;
					if ( level.round_number == level.mix_round && mixed_spawns < max_dogs && !level.bosses_enabled )
					{
						if ( RandomInt(100) <= 40 )
						{
							spawn_dog = true;
					...
					if(getdvarint("realism_bosses") == 1)
					{
						if( level.bosses_enabled && randomint(100) <= 50 )
						{
							spawn_dog = true;
						}
					}
					...
					if ( spawn_dog )
					{
						...
						for ( i=0; i<keys.size; i++ )
						{
							...
							{
								maps\factory\_zombiemode_dogs::special_dog_spawn( undefined, 1 );
								level.zombie_total--;
								wait_network_frame();
...
tom_mixed_dog_crap()
{
	maps\factory\_zombiemode_dogs::dog_health_increase();
...
play_intro_VO()
{
	...
	for(i=0;i<players.size;i++)
	{
		index = maps\factory\_zombiemode_weapons::get_player_index(players[i]);
		player_index = "plr_" + index + "_";
		sound_to_play = "vox_name_int_0";
		players[i] 	maps\factory\_zombiemode_spawner::do_player_playdialog(player_index, sound_to_play, 0.05);
		wait(0.7);
	}
...
round_think()
{
	for( ;; )
	{
		...
		maps\factory\_zombiemode_powerups::powerup_round_start();

		players = get_players();
		array_thread( players, maps\factory\_zombiemode_blockers_new::rebuild_barrier_reward_reset );
...
compile_bonus()
{
	...
	rampage_gethighest(arr,"downs1");
	rampage_gethighest(arr,"revives1"); 
	rampage_gethighest(arr,"headshots1");
	rampage_gethighest(arr,"knives1");
}

rampage_gethighest(a,type)
{
	temp = undefined;
	count = 0;
	for(i = 0; i < a.size; i++)
	{
		for(j = 0; j < a.size-1-i; j++)
		{
			if(type == "downs1")
			{
				if(a[i].downs1 == a[j].downs1)
					count++;
				if(a[j].downs1 == a[j+1].downs1)
					count++;
			...
			else if(type == "revives1")
			{
				if(a[i].revives1 == a[j].revives1)
					count++;
				if(a[j].revives1 == a[j+1].revives1)
					count++;
			...
			else if(type == "headshots1")
			{
				if(a[i].headshots1 == a[j].headshots1)
					count++;
				if(a[j].headshots1 == a[j+1].headshots1)
					count++;
			...
			else if(type == "knives1")
			{
				if(a[i].knives1 == a[j].knives1)
					count++;
				if(a[j].knives1 == a[j+1].knives1)
					count++;
	...
	if(count < a.size)
	{
		if(type == "downs1")
		{
			level.hardest2kill = a[0];
			level.hardest2kill.hardest2kill = true;
		}
		else if(type == "revives1")
		{
			level.mostRevives = a[a.size-1];
			level.mostRevives.mostrevives = true;
		}
		else if(type == "headshots1")
		{
			level.mostHeadshots = a[a.size-1];
			level.mostHeadshots.mostheadshots = true;
		}
		else if(type == "knives1")
		{
			level.mostKnives = a[a.size-1];
			level.mostKnives.mostknives = true;
		}
	}
}
...
round_bonus()
{
	...
	pts = randomIntRange(50,100);
	has_bonus = false;
	
	if( self.mostheadshots )
	{
		has_bonus = true;
		
		iprintlnbold("^1"+self.playername+" was Top Headhunter with "+self.headshots1+" headshots");
	
		self maps\_zombiemode_score::add_to_player_score( 250 );
	...
	if( self.mostknives )
	{
		...
		iprintlnbold("^2"+self.playername+" was Top Knife Killer with "+self.knives1+" knife kills");
	...
	if( self.mostrevives )
	{
		...
		iprintlnbold("^3"+self.playername+" was Top Lifesaver with "+self.revives1+" revives");
	
		self maps\_zombiemode_score::add_to_player_score( 150 );
		self playSound( "cha_ching" );
	}
	
	if( self.hardest2kill )
	{
		has_bonus = true;
		
		iprintlnbold("^4"+self.playername+" was Hardest2Kill with "+self.downs1+" downs");
	...
	self.revives1 = 0;
	self.knives1 = 0;
	self.mostheadshots = false;
	self.mostrevives = false;
	self.hardest2kill = false;
	self.mostknives = false;
	
	if(has_bonus)
		self maps\_challenges_coop::giveRankXP( "kill", pts );
}
...
award_grenades_for_survivors()
{
	...
	for (i = 0; i < players.size; i++)
	{
		if (!players[i].is_zombie)
		{
			if(players[i] hasWeapon("zombie_cymbal_monkey"))
			{
				players[i] setWeaponAmmoClip("zombie_cymbal_monkey", 3);
			}
...
zombify_player()
{
	...
	self thread maps\factory\_zombiemode_spawner::zombie_eye_glow(); 
...
playerzombie_downed_state()
{
	...
	self thread maps\factory\_zombiemode_spawner::zombie_eye_glow_stop(); 
	self DisableWeapons(); 
	self AllowStand( false ); 
	self AllowCrouch( false ); 
...
player_damage_override( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, modelIndex, psOffsetTime )
{
	...
	if(self HasPerk("specialty_quickrevive"))
	{
		if(self.quickrevive_up2 > 0)
		{
			...
			self thread maps\_rampage_util::hud_print_self( "You Have A Second Chance" );
...
player_update_time_played()
{
	self endon("disconnect");
	self endon("stop_tracking_time");
	...
	while(1)
	{
		wait(1);
		self.time_played = 1;
		self maps\_challenges_coop::statAdd("time_played_total",self.time_played);
	}
}
...
zombie_game_over_death()
{
	...
	for( i = 0; i < zombies.size; i++ )
	{
		...
		zombies[i] maps\factory\_zombiemode_spawner::zombie_head_gib();
