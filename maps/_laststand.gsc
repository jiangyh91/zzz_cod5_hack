...
laststand_giveback_player_weapons()
{
	...
	for( i = 0; i < self.weaponInventory.size; i++ )
	{
		...
		if(isDefined(self.grenade_secondary) && self.grenade_secondary != "")
		{
			if(!self hasWeapon(self.grenade_secondary))
			{
...
can_revive( revivee )
{
	...
	if(self.class_picked != "medic_1")
	{
		...
		if(isDefined(self.ability) && self.ability == "a_medic")
		{
			return true;
		}
...
revive_success( reviver )
{
	...
	reviver.revives++;
	reviver.revives1 += 1;
	reviver.medic_heals += 3;
	reviver setstat(313, reviver getstat(313)+3);
	if(reviver getstat(313) >= 75)
	{
		reviver setstat(1249, reviver getstat(1249)+1);
		reviver iprintlnbold("^2You received an upgrade point!");
		reviver.medic_heals = 0;
		reviver setstat(313, 0);
	}
	//stat tracking
