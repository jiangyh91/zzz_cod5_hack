...
////---------------------------------------////
	...
	maps\_rampage_util2::rampage_variables();
	...
	//maps\_rampage_drones::setup_drones();
	...
	maps\_zombiemode_weapons_prototype::init();
	maps\_zombiemode_blockers_prototype::init();
	maps\_zombiemode_spawner_prototype::init();
	maps\_zombiemode_powerups_prototype::init();
	
	if(getdvar("mapname") == "nazi_zombie_zct_bsmnt" ||
	   getdvar("mapname") == "nazi_zombie_zct_bunker" ||
	   getdvar("mapname") == "nazi_zombie_zct_dhouse" ||
	   getdvar("mapname") == "nazi_zombie_zct_dshack" ||
	   getdvar("mapname") == "zombie_rich")
	{
		maps\_zombiemode_perks_woo::main();
		
		water_trigs = getentarray( "waterfall","targetname" );
		if(water_trigs.size >= 1)
			array_thread( water_trigs,maps\woo_toolkit::watersheet_on_trigger );
			
		if(getdvar("mapname") != "zombie_rich")
		{
			maps\woo_toolkit::recharge_station_init();
			maps\woo_toolkit::cleanup_tags();
		}
	}
...
//	level.global_kill_func = maps\_zombiemode_spawner::zombie_death; 
	...
	// Modified by Rampage - wait until at least one player is actually playing
...
round_spawning()
{
	...
	if(!level.tom_use_timer)
	{
		while( count < max )
		{
			...
			spawn_points = get_array_of_closest( player.origin, level.enemy_spawns, undefined, 6 );
...
round_start()
{
	...
	for (i = 0; i < players.size; i++)
	{
		...
		players[i] setweaponammoclip( "stielhandgranate", players[i].grenade_count);
	}
	*/
...
while(1)
	...
	if(level.zombie_vars["zombie_spawn_delay"] > 1.1)
		...
		maps\_zombiemode_powerups_prototype::powerup_round_start();
		array_thread( players, maps\_zombiemode_blockers_prototype::rebuild_barrier_reward_reset );
...
player_fake_death()
{
	...
	self setclientdvar("rampage_show_health","0");
